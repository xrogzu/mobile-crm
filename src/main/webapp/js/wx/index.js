...
$(document).bind('ready', function() {
    ...
    $(".swiper-container").swiper({
        ...
        onSlideChangeEnd: function(swiper) {
            var performancePage = $("#performancePage");
            if (performancePage.hasClass("swiper-slide-active")) {
                $(".performance").addClass("active");
                $(".performance").siblings().removeClass("active");
                tabChange();
            }
            var behaviorPage = $("#behaviorPage");
            if (behaviorPage.hasClass("swiper-slide-active")) {
                $(".behavior").addClass("active");
                $(".behavior").siblings().removeClass("active");
                tabChange();
            }
        }
    });
...
function tabChange() {
    var schedulelist = $(".schedule-item").find('.schedule-cell');
    var tav = $("#sort").find(".behavior").hasClass("active");
    if (schedulelist.length < 5 && tav) {
        // $(".index-body.swiper-wrapper").css("overflow", 'hidden');
    } else {
        // $(".index-body.swiper-wrapper").css("overflow", '');
    }
}
...
function sort() {
    ...
    $('#sort .behavior').bind('click', function() {
        ...
        if (!$(this).hasClass('active')) {
            ...
            $(".swiper-wrapper").css("-webkit-transition-duration", "0ms");
            ...
            $('#sort .performance').removeClass('active');
            tabChange();
            $('#behaviorPage').addClass("swiper-slide-active");
            $('#performancePage').removeClass("swiper-slide-active");
        }
    })
    $('#sort .performance').bind('click', function() {
        if (!$(this).hasClass('active')) {
            $(this).addClass('active');
            var transformwidth = -Number(indexWidth )+ "px";
            $(".swiper-wrapper").css("-webkit-transition-duration", "0ms");
            $(".swiper-wrapper").css("-webkit-transform", "translate3d("+transformwidth+", 0px, 0px)");
            $(".swiper-wrapper").css("transition-duration", "0ms");
          
            $(".swiper-wrapper").css("transform", "translate3d("+transformwidth+", 0px, 0px)");
            $('#sort .behavior').removeClass('active');
            tabChange();
            $('#performancePage').addClass("swiper-slide-active");
            $('#behaviorPage').removeClass("swiper-slide-active");
        }
    })
}
...
function change_funnel() {
    $('.funnelTop .btn').bind('click', function() {
        if ($(this).text() == '个数') {
            $(this).text('金额');
    ...
    $.ajax({
        ...
        success: function(oData) {
            ...
            if (oData.success == true) {
                ...
                ranking(oData);
                if(flag == true){
                    $(".ranking span.count").html(oData.entity.totalCount);
                }
...
function ranking(data) {
    ...
    if (data.entity.series.length > 10) {
        ...
        $('.searchMore').bind('click', function() {
            ...
            checkTable('业绩排名');
            $('.ct').addClass('ranking');
            var top = $('<div class="title boxSizing"><p class="label">业绩总值</p><p class="value"><span class="count">' + data.entity.totalCount + '</span><span>元</span></p></div>');
...
//     $(".ranking .selector li").on('click'.function(){
//         alert("xxx");
//     });
// }
//跟进记录
...
function oppListByData(oData, $container, build_unit, scrollFunc, type) {
    ...
    if (len == 0) {
        //无结果情况
        $container.append($('<div class="noInfo">该范围内没有重点商机</div>'));
    } else {
        //有结果情况
...
function workreportSelect() {
    $('#workreport_select .options li').on('click', function() {
        var workreportDateType = $(this).attr('value');
        var text = $(this).text();
        $('.workreport-select-date').html(text);
        $('.workreport-select-date').attr('value', workreportDateType);
        workreportSum(workreportDateType);
    });
}

function workreportSum(DateType) {
    //先加载缓存数据
    if (localStorage.getItem(xsyUser.id)) {
        var ls = JSON.parse(localStorage.getItem(xsyUser.id));
        if (ls.index) {
            if (ls.index.workreportDate) {
                if (ls.index.workreportDate.searchDateType != "") {
                    //alert('加载rank缓存数据')
                    // workreportSum(ls.index.workreportDate);
                }
            }
        }
    }
    $.ajax({
        url: apppath + '/wx/statics/staticinfo.action',
        data: {
        ...
        success: function(oData) {
            // var oData = eval('(' + data + ')');
            if (oData.success == true) {
                $('#maccount_num').html(oData.entity.newAddAccouontNum);
                $('#account_num').html(oData.entity.allAccouontNum);
                $('#mcontact_num').html(oData.entity.newAddContactNum);
                $('#contact_num').html(oData.entity.AllContactNum);
                $('#mopp_num').html(oData.entity.newAddOpportunityNum);
                $('#opp_num').html(oData.entity.AllOpportunityNum);
                workreportUrl();
                var ls;
                if (localStorage.getItem(xsyUser.id)) {
                    ls = JSON.parse(localStorage.getItem(xsyUser.id));
                    ls.index = { 'workreportDate': oData };
                } else {
                    ls = { 'index': { 'workreportDate': oData } }
                }
                localStorage.setItem(xsyUser.id, JSON.stringify(ls));
...
function addschdule() {
    $('.index-add-schdule').on('click', function() {
        location.href = apppath + '/wx/schedule/add.action?' + ddcommparams + '&from=index_list&type=add';
    });
}
...
function workreportUrl() {
    var entitysearchDateType = $('.workreport-select-date').attr('value');
    $("#acount_account").on('click', function() {
        location.href = apppath + '/wx/account/list.action?' + ddcommparams;
    });
    $("#acount_contact").on('click', function() {
        location.href = apppath + '/wx/account/list.action?' + ddcommparams;
    });
    $("#acount_opp").on('click', function() {
        location.href = apppath + '/wx/opportunity/list.action?' + ddcommparams;
    });
    $("#count_account").on('click', function() {
        location.href = apppath + '/wx/statics/account.action?' + ddcommparams + '&searchDateType=' + entitysearchDateType;
    });
    $("#count_contact").on('click', function() {
        location.href = apppath + '/wx/statics/contact.action?' + ddcommparams + '&searchDateType=' + entitysearchDateType;
    });
    $("#count_opp").on('click', function() {
        location.href = apppath + '/wx/statics/opportunity.action?' + ddcommparams + '&searchDateType=' + entitysearchDateType;
    });

}

//获取今天日程列表
function getScduleinfo() {
    ...
    if (localStorage.getItem(xsyUser.id)) {
        var ls = JSON.parse(localStorage.getItem(xsyUser.id));
        if (ls.index) {
            if (ls.index.scheduleDate) {
                if (ls.index.scheduleDate.id != "") {
                    //alert('加载rank缓存数据')
                    createScduleList(ls.index.scheduleDate);
                }
            }
        }
    }
            ...
            if (data.success == false) {
                ...
                $.each(records, function(i, o) {
                    if (o.count == 0) {
                        ...
                        var schedulenull = $('<div class="schedule-body-null"><div class="schedule-null-content"><span class="null-ico"></span><p>今天没有日程哦</p><a href="javascript:;" class="index-add-schdule">马上添加</a></div></div>')
...
function createScduleList(data) {
    ...
    $(".schedule-item .schedule-cell").on('click', function() {
        var id = $(this).attr("id");
        location.href = apppath + '/wx/schedule/info.action?' + ddcommparams + '&from=index_schdule_list' + '&id=' + id;
    });
    schduleAccept();
    scheduleReject();
}
