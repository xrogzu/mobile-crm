...
public class StaticsAction extends BaseAction{
    ...
    @Action(value = "index", results = {@Result(name = SUCCESS, location = PAGES_ROOT+"/dingtalk/statics/statics_index.jsp")})
    public String list() throws Exception{
        return  SUCCESS;
    }
    @Action(value = "condition", results = {@Result(name = SUCCESS, location = PAGES_ROOT+"/dingtalk/statics/statics_condition.jsp")})
    ...
    public String doStatisActiveRecord()  {
        ...
        EntityReturnData ret = new EntityReturnData();
        try {
            DingSessionUser dingSessionUser = getDingSessionUser();
            TenantParam tenantParam =  dingSessionUser.getXsy_TenantParam();
            XsyUserDto xsyUserDto = dingSessionUser.getXsy_user();

//            TenantParam tenantParam = new TenantParam(40537L);
//            XsyUserDto xsyUserDto = new XsyUserDto();
//            xsyUserDto.setId(74404L);
//            String access_token = "54d9809102e05e0e166f07669c588aac48c25385bac55f26840888c528eb51fd";
...
//                pageData.put("totalPage", totalPage);
    ...
    public String getActiverecordlist() {
        ...
        try {
            ...
            JSONArray dimDepartArray = null;
             if(dingSessionUser.isNormal()){
                 ownerId =  dingSessionUser.getXsy_user().getId();
            }else if(dingSessionUser.isManager()){
    ...
    private void getMyDepartmetnIdJSONArray(JSONArray departmetnJSONArray,JSONObject departmentObject){

        departmetnJSONArray.add(departmentObject.getLong("id"));
        if(departmentObject.containsKey("subs")){
            JSONArray subs = departmentObject.getJSONArray("subs");
            for(int i=0;i<subs.size();i++){
                JSONObject item = subs.getJSONObject(i);
                /**
                 * 递归调用，获取部门id
                 */
                getMyDepartmetnIdJSONArray(departmetnJSONArray,item);
            }
        }
